...
export default function FaceCam({ onEmotion }: Props) {
  ...
  const detect = useCallback(() => {
    ...
    // Only process when a genuinely new video frame is available (official MediaPipe pattern)
    ...
    let results: any;
    try {
      results = landmarker.detectForVideo(v, Date.now());
    } catch {
      rafRef.current = requestAnimationFrame(detect);
      return;
    }
    ...
    const smoothed = smoothRef.current
      ? (() => {
          const r: any = {};
          (Object.keys(emo) as (keyof EmotionScores)[]).forEach(k => {
            r[k] = 0.25 * emo[k] + 0.75 * smoothRef.current![k];
          });
    ...
    if (now - lastSend.current > SEND_INTERVAL) {
      ...
      onEmotionRef.current?.(smoothed, score, openness);
            ...
            <div className="w-12 h-12 rounded-full flex items-center justify-center"
              ...
              {phase === "loading"
                ? <motion.div animate={{ rotate: 360 }} transition={{ duration: 1, repeat: Infinity, ease: "linear" }}>
                    <Loader size={22} color="var(--amber)" />
                  </motion.div>
              ...
              <div className="rounded-lg p-2" style={{ background: "var(--surface-2)", border: "1px solid var(--border)" }}>
                ...
                <div className="h-1.5 rounded-full overflow-hidden" style={{ background: "var(--border)" }}>
                  <motion.div className="h-full rounded-full" style={{ background: eyeColor }}
                    initial={{ width: "0%" }} animate={{ width: `${(eyeOpen * 100).toFixed(1)}%` }}
                    transition={{ duration: 0.3 }} />
                </div>
                <div className="flex justify-between mt-1">
                  <span className="text-xs" style={{ color: eyeColor }}>{eyeStatus}</span>
                  <span className="text-xs" style={{ color: "var(--muted)" }}>PERCLOS {(perclos * 100).toFixed(0)}%</span>
                </div>
              </div>
              ...
              {pose && (
                <div className="rounded-lg p-2" style={{ background: "var(--surface-2)", border: "1px solid var(--border)" }}>
                  ...
                  <div className="grid grid-cols-3 gap-1">
                    {([["P", pose.pitch, "#60a5fa"], ["Y", pose.yaw, "#f59e0b"], ["R", pose.roll, "#a855f7"]] as const).map(([l, v, c]) => (
                      <div key={l} className="text-center">
                        <div className="text-xs font-semibold" style={{ color: c }}>{v.toFixed(0)}°</div>
